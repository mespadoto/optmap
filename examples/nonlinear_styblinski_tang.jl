...
function solve_model(solver, model_log, solver_options=Dict())
    ...
    @variable(model, -4.0 <= x[1:n_variables] <= 5.0)
    @NLconstraint(model, sum((x[i]^2 for i in 1:n_variables)) <= 5.0)

    constraints_f = []
    push!(constraints_f, (v) -> sum(v.^2) <= 5.0)

    @NLobjective(model, Min, sum(((x[i]^4 - 16x[i]^2 + 5x[i]) / 2 for i in 1:n_variables)))
    obj_f(x) = sum(x.^4 - 16x.^2 + 5x) / 2

    starting_point = zeros(Float64, n_variables) .+ 4.0
...
for (opt, opt_name) in zip(optimizers, opt_names)
    objective_f, constraints_f, variable_names, constraint_names, starting_point, solution_point, obj_val = solve_model(opt, "styblinski_tang_$(opt_name).log")
...
for opt_name in opt_names
    ...
    save_all(O, "styblinski_tang_$(opt_name)", ".")
end
